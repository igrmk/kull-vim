hi clear

if exists('syntax_on')
    syntax reset
endif

let g:colors_name='kull'
set background=dark

hi Normal           ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE

hi Cursor           ctermfg=white ctermbg=NONE cterm=NONE        guifg=#eeeeec guibg=NONE    gui=NONE
hi IncSearch        ctermfg=108   ctermbg=NONE cterm=bold        guifg=#87af87 guibg=NONE    gui=bold
hi Search           ctermfg=110   ctermbg=NONE cterm=bold        guifg=#87afd7 guibg=NONE    gui=bold
hi Visual           ctermfg=110   ctermbg=235  cterm=reverse     guifg=#87afd7 guibg=#262626 gui=reverse
hi VisualNOS        ctermfg=white ctermbg=NONE cterm=reverse     guifg=#eeeeec guibg=NONE    gui=reverse

hi Constant         ctermfg=108   ctermbg=NONE cterm=bold        guifg=#87af87 guibg=NONE    gui=bold
hi DiffAdd          ctermfg=108   ctermbg=NONE cterm=NONE        guifg=#87af87 guibg=NONE    gui=NONE
hi Number           ctermfg=108   ctermbg=NONE cterm=bold        guifg=#87af87 guibg=NONE    gui=bold
hi String           ctermfg=108   ctermbg=NONE cterm=bold        guifg=#87af87 guibg=NONE    gui=bold

hi CursorColumn     ctermfg=white ctermbg=NONE cterm=NONE        guifg=#eeeeec guibg=NONE    gui=NONE
hi CursorLine       ctermfg=white ctermbg=NONE cterm=NONE        guifg=#eeeeec guibg=NONE    gui=NONE
hi CursorLineNr     ctermfg=white ctermbg=NONE cterm=NONE        guifg=#eeeeec guibg=NONE    gui=NONE

hi Comment          ctermfg=243   ctermbg=NONE cterm=italic      guifg=#767676 guibg=NONE    gui=italic
hi DiffChange       ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi DiffDelete       ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi DiffText         ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi Directory        ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi Error            ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi ErrorMsg         ctermfg=0     ctermbg=1    cterm=NONE        guifg=#2e3436 guibg=#cb5555 gui=NONE
hi Folded           ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi Function         ctermfg=248   ctermbg=NONE cterm=bold        guifg=#a8a8a8 guibg=NONE    gui=bold
hi Identifier       ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi Ignore           ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi Label            ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi LineNr           ctermfg=240   ctermbg=NONE cterm=NONE        guifg=#585858 guibg=NONE    gui=NONE
hi MatchParen       ctermfg=229   ctermbg=241  cterm=NONE        guifg=#ffffaf guibg=#626262 gui=NONE
hi ModeMsg          ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi MoreMsg          ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi NonText          ctermfg=240   ctermbg=NONE cterm=NONE        guifg=#585858 guibg=NONE    gui=NONE
hi Pmenu            ctermfg=248   ctermbg=235  cterm=NONE        guifg=#a8a8a8 guibg=#262626 gui=NONE
hi PmenuSbar        ctermfg=248   ctermbg=131  cterm=NONE        guifg=#a8a8a8 guibg=#af5f5f gui=NONE
hi PmenuSel         ctermfg=235   ctermbg=108  cterm=bold        guifg=#262626 guibg=#87af87 gui=bold
hi PmenuThumb       ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi PreProc          ctermfg=248   ctermbg=NONE cterm=bold        guifg=#a8a8a8 guibg=NONE    gui=bold
hi Special          ctermfg=248   ctermbg=NONE cterm=bold,italic guifg=#a8a8a8 guibg=NONE    gui=bold,italic
hi SpecialKey       ctermfg=238   ctermbg=NONE cterm=NONE        guifg=#444444 guibg=NONE    gui=NONE
hi Statement        ctermfg=248   ctermbg=NONE cterm=bold        guifg=#a8a8a8 guibg=NONE    gui=bold
hi StatusLine       ctermfg=248   ctermbg=239  cterm=bold        guifg=#a8a8a8 guibg=#4e4e4e gui=bold
hi StatusLineNC     ctermfg=246   ctermbg=239  cterm=NONE        guifg=#949494 guibg=#4e4e4e gui=NONE
hi StorageClass     ctermfg=248   ctermbg=NONE cterm=bold        guifg=#a8a8a8 guibg=NONE    gui=bold
hi Structure        ctermfg=248   ctermbg=NONE cterm=bold        guifg=#a8a8a8 guibg=NONE    gui=bold
hi TabLine          ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi TabLineFill      ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi TabLineSel       ctermfg=248   ctermbg=NONE cterm=bold        guifg=#a8a8a8 guibg=NONE    gui=bold
hi Title            ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi Todo             ctermfg=254   ctermbg=100  cterm=bold        guifg=#e4e4e4 guibg=#878700 gui=bold
hi Type             ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi TypeDef          ctermfg=248   ctermbg=NONE cterm=bold        guifg=#a8a8a8 guibg=NONE    gui=bold
hi Underlined       ctermfg=248   ctermbg=NONE cterm=underline   guifg=#a8a8a8 guibg=NONE    gui=underline
hi VertSplit        ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi WarningMsg       ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi WildMenu         ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi cucumberTags     ctermfg=248   ctermbg=NONE cterm=bold        guifg=#a8a8a8 guibg=NONE    gui=bold
hi htmlTagN         ctermfg=248   ctermbg=NONE cterm=bold        guifg=#a8a8a8 guibg=NONE    gui=bold
hi rubySymbol       ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi vimHiTerm        ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi vimGroupName     ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi CurrentWordTwins ctermfg=248   ctermbg=240  cterm=NONE        guifg=#a8a8a8 guibg=#585858 gui=NONE
hi Delimiter        ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi Conceal          ctermfg=108   ctermbg=NONE cterm=bold        guifg=#87af87 guibg=NONE    gui=bold

hi goParamName      ctermfg=248   ctermbg=NONE cterm=bold,italic guifg=#a8a8a8 guibg=NONE    gui=bold,italic
hi goParamType      ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi goFunctionCall   ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE

hi texSpecialChar   ctermfg=243   ctermbg=NONE cterm=NONE        guifg=#767676 guibg=NONE    gui=NONE
hi texStatement     ctermfg=248   ctermbg=NONE cterm=NONE        guifg=#a8a8a8 guibg=NONE    gui=NONE
hi texMath          ctermfg=108   ctermbg=NONE cterm=bold,italic guifg=#87af87 guibg=NONE    gui=bold,italic
hi texSuperscript   ctermfg=108   ctermbg=NONE cterm=bold,italic guifg=#87af87 guibg=NONE    gui=bold,italic
hi texSubscript     ctermfg=108   ctermbg=NONE cterm=bold,italic guifg=#87af87 guibg=NONE    gui=bold,italic

hi SignColumn ctermfg=NONE ctermbg=NONE cterm=NONE guifg=NONE guibg=NONE gui=NONE

hi SpellBad   ctermfg=218  ctermbg=NONE cterm=underline guifg=#ffafd7 guibg=NONE guisp=NONE gui=underline
hi SpellCap   ctermfg=NONE ctermbg=NONE cterm=underline guifg=NONE    guibg=NONE guisp=NONE gui=underline
hi SpellRare  ctermfg=NONE ctermbg=NONE cterm=underline guifg=NONE    guibg=NONE guisp=NONE gui=underline
hi SpellLocal ctermfg=NONE ctermbg=NONE cterm=underline guifg=NONE    guibg=NONE guisp=NONE gui=underline

hi EasyMotionTarget        ctermbg=108  ctermfg=235 cterm=bold guibg=#87af87 guifg=#262626 gui=bold
hi EasyMotionTarget2First  ctermbg=108  ctermfg=235 cterm=bold guibg=#87af87 guifg=#262626 gui=bold
hi EasyMotionTarget2Second ctermbg=108  ctermfg=235 cterm=bold guibg=#87af87 guifg=#262626 gui=bold
hi EasyMotionShade         ctermbg=NONE ctermfg=242 cterm=NONE guibg=NONE    guifg=#6c6c6c gui=NONE
hi EasyMotionIncSearch     ctermbg=NONE ctermfg=110 cterm=bold guibg=NONE    guifg=#87afd7 gui=bold
hi EasyMotionMoveHL        ctermbg=NONE ctermfg=108 cterm=bold guibg=NONE    guifg=#87af87 gui=bold

hi FlashLabel              ctermbg=108  ctermfg=235 cterm=bold guibg=#87af87 guifg=#262626 gui=bold
hi FlashBackdrop           ctermbg=NONE ctermfg=242 cterm=NONE guibg=NONE    guifg=#6c6c6c gui=NONE

if has('gui_running') || exists('g:neovide')
    hi Normal guibg=#252525
    hi Cursor guifg=#000000 guibg=#729fcf
endif
